...
const { state, withTimeout, shutdown } = require('./state');
const out = require('./output');
const { commands, watchPage, complete } = require('./commands');
...
async function start(options) {
  ...
  const rl = readline.createInterface({
    ...
    prompt: options.serve ? 'pw[serve]> ' : 'pw> ',
    completer: complete,
    historySize: 1000,
  ...
  rl.on('SIGINT', () => shutdown().then(() => process.exit(process.exitCode || 0)));
}

// SIGHUP too: closing the terminal must still remove the server's socket.
for (const signal of ['SIGTERM', 'SIGHUP']) process.on(signal, () => shutdown().then(() => process.exit(process.exitCode || 0)));
