...
// Prompt commands report completion with a marker the caller can wait for.
...
const READ_ONLY = new Set(['tabs', 'info', 'title', 'url', 'text', 'html', 'attrs', 'count', 'visible', 'links', 'inputs',
  'snapshot', 'screenshot', 'wait', 'sleep', 'recent', 'body', 'logs', 'routes', 'cookies', 'storage', 'network', 'console',
  'capture', 'stop', 'help']);

// Commands that work with no tab selected.
const NO_TAB_NEEDED = new Set(['tabs', 'tab', 'help', 'quit']);

// Commands that accept a leading --all to lift the output limit.
const INSPECTION = ['info', 'title', 'url', 'text', 'html', 'attrs', 'links', 'inputs', 'eval', 'raw', 'cookies', 'storage', 'capture', 'stop', 'body', 'logs', 'snapshot', 'watch'];

let completionId = Date.now();
let queue = Promise.resolve();

function parseInput(line) {
  const trimmed = line.trim();
  const match = /^@([A-Za-z0-9][A-Za-z0-9._-]*)\s+([\s\S]+)$/.exec(trimmed);
  return { text: match ? match[2] : trimmed, token: match ? match[1] : `auto-${++completionId}` };
}
...
function giveUp() {
  ...
  void shutdown().then(() => process.exit(process.exitCode || 1));
}

async function handleLine(line) {
  const input = parseInput(line);
  if (state.stopping) {
    out.log(`[[pw-done:${input.token}:error]]`);
    return;
  }
  const result = await execute(input.text);
  out.log(`[[pw-done:${input.token}:${result.status}]]`);
  if (result.cmd === 'quit') process.exit(process.exitCode || 0);
  if (result.uncertain) return giveUp();
  if (!state.stopping) state.rl.prompt();
}
  ...
  return new Promise(resolve => {
    queue = queue.then(async () => {
      ...
      if (/^quit(?:\s|$)/.test(text)) {
        ...
        resolve({ status: result.status, output, uncertain: result.uncertain });
      }
      if (!state.stopping) state.rl.prompt(true);
...
module.exports = { enqueue, submit, giveUp, drained, busy };
