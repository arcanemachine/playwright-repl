...
function paneProblem(session) {
  if (!hasSession(session)) return `no tmux session '${session}'; ask the user before creating one (AGENTS.md: Start it)`;
  const running = tmux('display-message', '-p', '-t', session, '#{pane_current_command}').trim();
  if (running !== 'node') return `the REPL is not running in '${session}' (pane is running '${running}'); start it there with pw-repl run (AGENTS.md: Start it)`;
...
async function viaTmux(session, command, timeoutMs) {
  ...
  const id = `pw-${Date.now()}${process.hrtime.bigint() % 1000000n}-${process.pid}`;
