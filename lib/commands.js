...
const WATCH_SCRIPT = `(() => {
  ...
  const send = (action, el, extra, since) => {
    if (typeof window.__pwReplWatch === 'function') window.__pwReplWatch({ action, target: describe(el), extra: extra || '', since: since || 0 });
  };
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Enter' && !(textEntry(el) || el.isContentEditable)) return;
    flushTyping(el);
    send('press', el, e.key);
  }, true);
...
async function snapshotText(p) {
  return p.ariaSnapshot({ mode: 'ai', timeout: SETTLE_MAX });
}
...
async function startWatching(p, changes = false) {
  ...
  if (!watch) {
    // shownSeq and shownRequestId mark how far reads have got, for watch new.
    const created = { on: false, changes: false, snapshot: null, settleTimer: null, events: [], nextSeq: 1, shownSeq: 0, shownRequestId: 0 };
  ...
  // The first step's changes are measured from how the page looks now.
  watch.snapshot = changes ? await snapshotText(p) : null;
  watch.on = true;
}
...
const commands = {
  ...
  async watch(args, all) {
    ...
    const count = arg && !onlyNew ? Number(arg) : RECENT_DEFAULT;
    ...
    const printStep = (e, caused, note = '') => {
      lines.push(`${clock(e.t)} ${e.action} ${e.target}${e.extra ? ` ${e.extra}` : ''}${note}`);
      ...
      if (e.changes && !e.changesShown) {
        for (const change of e.changes) lines.push(`${indent}${change}`);
        e.changesShown = true;
      }
    };
    let events;
...
module.exports = { commands, watchPage, complete, compactSnapshot, grepSnapshot, summarizeChanges, clock };
