...
const commands = {
  ...
  async watch(args, all) {
    ...
    const printStep = (e, caused, note = '') => {
      ...
      if (e.changes && !(onlyNew && e.changesShown)) {
        for (const change of e.changes) lines.push(`${indent}${change}`);
        if (onlyNew) e.changesShown = true;
      }
    };
    let events;
    if (onlyNew) {
...
module.exports = { commands, watchPage, complete, compactSnapshot, grepSnapshot, summarizeChanges, scrubEditable, clock };
