...
// Shared by network/console/capture. With seconds: blocking timed window.
// Without: returns to the prompt and captures until 'stop'.
async function runCapture(tags, args, label) {
  if (cap) { out.log("Already capturing — type 'stop' to finish."); return; }
  const duration = (args || '').trim();
  const seconds = duration ? Number(duration) : null;
  if (duration && (!Number.isInteger(seconds) || seconds < 1 || seconds > 3600)) throw new Error('Capture duration must be an integer from 1 to 3600 seconds');
  ...
  if (tags.includes('net')) {
    const h = req => record({ t: Date.now(), tag: 'net', text: `${req.method()} ${req.url()}` });
    state.page.on('request', h);
    handlers.push(['request', h]);
  }
  if (tags.includes('console')) {
    const h = msg => record({ t: Date.now(), tag: `console:${msg.type()}`, text: msg.text() });
    state.page.on('console', h);
    handlers.push(['console', h]);
  }
  cap = {
    page: state.page, startedAt, events, handlers,
  ...
  if (seconds) {
    ...
    out.log(`Capturing ${label} — type 'stop' to finish.`);
  }
}
...
const PAGE_ACTIONS = new Set(['click', 'check', 'uncheck', 'select', 'fill', 'submit']);
...
const WATCH_SCRIPT = `(() => {
  ...
  const nameOf = el => {
    ...
    return clip(el.getAttribute('aria-label') || byId || (label && label !== el && label.innerText) || el.getAttribute('alt')
      || el.getAttribute('placeholder') || el.getAttribute('title') || (typedInto(el) ? '' : el.innerText));
  };
  const describe = el => { const name = nameOf(el); return roleOf(el) + (name ? ' ' + JSON.stringify(name) : ''); };
  const send = (action, el, extra) => {
    if (typeof window.__pwReplWatch === 'function') window.__pwReplWatch({ action, target: describe(el), extra: extra || '' });
  };
...
async function startWatching(p) {
  let watch = watches.get(p);
  if (!watch) {
    const created = { on: false, events: [] };
    const record = (action, target, extra) => {
      if (created.on) keep(created.events, { t: Date.now(), action, target: clipText(target), extra: clipText(extra) });
    };
    ...
    if (!watchBindings.has(p)) {
      ...
      await p.exposeBinding('__pwReplWatch', (_source, event) => {
        if (binding.record && event && PAGE_ACTIONS.has(event.action)) binding.record(event.action, String(event.target || ''), String(event.extra || ''));
      });
...
function showCapture(all = false) {
  if (!lastCapture) { out.log('No completed capture.'); return; }
...
const commands = {
  ...
  async tab(args) {
    ...
    if (!trimmed) {
      if (!state.page) { out.log('No tab is selected'); return; }
    ...
    if (/^\d+$/.test(subcommand)) {
      ...
      if (!target || !all.includes(target)) throw new Error('That tab is unavailable. Run tabs again.');
      select(target);
      return commands.info();
    }
    if (subcommand === 'new') {
    ...
    if (subcommand === 'close') {
      ...
      if (!target || target.isClosed() || !all.includes(target)) throw new Error('Selected tab is unavailable. Run tabs again.');
  ...
  async title() { out.log(await state.page.title()); },
  async url() { out.log(state.page.url()); },
  ...
  async offline(args) {
    ...
    if (arg !== 'on' && arg !== 'off') throw new Error('Usage: offline on|off');
    let session = offlineSessions.get(state.page);
    if (!session) {
      session = await state.page.context().newCDPSession(state.page);
      await session.send('Network.enable');
      offlineSessions.set(state.page, session);
    }
    ...
    out.log(`Network ${arg === 'on' ? 'offline' : 'online'}`);
  },
  ...
  async route(args) {
    const match = /^(\S+)\s+(\d{3})\s+([\s\S]+)$/.exec((args || '').trim());
    if (!match) throw new Error('Usage: route <url-glob> <status> <json-body>');
  ...
  async unroute(args) {
    ...
    if (!glob || /\s/.test(glob)) throw new Error('Usage: unroute <url-glob> | unroute --all');
    const routes = routesFor(state.page);
    ...
    if (!targets.length) out.log('No routes on the selected tab');
  },
  ...
  async recent(args) {
    ...
    if (count < 1 || count > RECENT_MAX || tokens.length > 1) throw new Error(`Usage: recent [--all] [n] [url-filter] (n from 1 to ${RECENT_MAX})`);
    ...
    if (!matches.length) { out.log(filter ? `No recent requests matching ${filter}` : 'No recent requests on the selected tab'); return; }
  ...
  async body(args, all) {
    ...
    if (!match) throw new Error('Usage: body <#> (the number from recent)');
    const id = Number(match[1]);
    const entry = (recentLogs.get(state.page) || []).find(e => e.id === id);
    if (!entry) throw new Error(`No request #${id} on the selected tab; recent lists them`);
  ...
  async logs(args, all) {
    ...
    let count = RECENT_DEFAULT;
    if (tokens.length && /^\d+$/.test(tokens[0])) count = Number(tokens.shift());
    if (count < 1 || count > RECENT_MAX || tokens.length > 1) throw new Error(`Usage: logs [n] [filter] (n from 1 to ${RECENT_MAX})`);
    const filter = tokens[0];
  ...
  async watch(args, all) {
    ...
    if (arg === 'on') {
      await startWatching(state.page);
      out.log('Watching the selected tab: clicks, form changes, submits and navigations (typed values are not recorded)');
      return;
    }
    if (arg === 'off') {
      if (watch) watch.on = false;
      out.log('Stopped watching the selected tab');
      return;
    }
    if (arg && !/^\d+$/.test(arg)) throw new Error('Usage: watch on | watch off | watch [n]');
    const count = arg ? Number(arg) : RECENT_DEFAULT;
    if (count < 1 || count > WATCH_MAX) throw new Error(`Usage: watch [n] (n from 1 to ${WATCH_MAX})`);
    ...
    const requests = (recentLogs.get(state.page) || []).filter(r => !RECENT_HIDDEN_TYPES.has(r.type) && !r.url.startsWith('chrome-extension://'));
    const events = watch.events.slice(-count);
    const lines = [];
    events.forEach((e, i) => {
      const next = watch.events[watch.events.indexOf(e) + 1];
      lines.push(`${clock(e.t)} ${e.action} ${e.target}${e.extra ? ` ${e.extra}` : ''}`);
      const caused = requests.filter(r => r.t >= e.t && (!next || r.t < next.t));
      ...
      if (caused.length > WATCH_REQUESTS_SHOWN) lines.push(`${indent}… ${caused.length - WATCH_REQUESTS_SHOWN} more (recent)`);
    });
  ...
  async raw(args, all) {
    ...
    if (!match) throw new Error('Usage: raw <CDP method> <JSON object>');
  ...
  async network(args) { await runCapture(['net'], args, 'network'); },

  async console(args) { await runCapture(['console'], args, 'console'); },

  async stop(_args, all) {
    if (!cap) { out.log('Nothing is capturing.'); return; }
    cap.handlers.forEach(([event, h]) => cap.page.off(event, h));
    const { startedAt, events, dropped, shortened } = cap;
    cap = null;
    lastCapture = { startedAt, events, dropped, shortened };
    printCapture(lastCapture, all);
  },

  async capture(args, all) {
    const trimmed = (args || '').trim();
    if (/^show(?:\s+--all)?$/.test(trimmed)) return showCapture(all || trimmed.endsWith('--all'));
    await runCapture(['net', 'console'], trimmed, 'network + console');
  },
...
function complete(line) {
  ...
  if (words.length === 2) {
    ...
    if (command === 'offline') return match(['on', 'off']);
    if (command === 'capture') return match(['show']);
    if (command === 'unroute') return match(['--all', ...(state.page ? routesFor(state.page).keys() : [])]);
  }
  return [[], current];
}

module.exports = { commands, watchPage, complete, compactSnapshot, grepSnapshot, clock };
