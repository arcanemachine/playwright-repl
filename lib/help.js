...
const TOPICS = {
  ...
  network: {
    intro: 'recent and logs record all the time; captures record only while running.',
    commands: ['recent', 'body', 'logs', 'network', 'console', 'capture', 'stop', 'route', 'unroute', 'routes', 'offline'],
  },
  devtools: {
...
const COMMANDS = {
  ...
  watch: {
    usage: 'watch on|off | watch [--all] [n]',
    summary: 'record what happens in the tab; show the last n steps',
    detail: 'Off until watch on. Records clicks, form changes, submits and navigations, each described by role and\nname like snapshot, with up to 5 of the requests it caused underneath\n(their recent numbers; body <#> for one).\nTyped values are not recorded, and password fields not at all. Nothing is replayed.',
  },
  ...
  logs: {
    ...
    detail: 'Recording starts when the REPL connects; the last 200 per tab are kept. Each line: time, [type], text.\nTypes are console levels (log, warning, error, ...) and pageerror for uncaught exceptions, which\nthe console capture does not see. filter matches the type or the text, e.g. logs error.',
  },
  network: { usage: 'network [seconds]', summary: 'capture request URLs for 1-3600s, or until stop' },
  console: { usage: 'console [seconds]', summary: 'capture console messages for 1-3600s, or until stop' },
  capture: {
    usage: 'capture [seconds] | capture show [--all]',
    summary: 'capture requests and console; show repeats the last one',
  },
  stop: { usage: 'stop [--all]', summary: 'end the running capture and print it' },
  route: {
