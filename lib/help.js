...
const COMMANDS = {
  ...
  capture: {
    ...
    detail: 'capture on records both until capture off, which prints them; requests or console records only one.\nWith secs (1-3600 seconds) it records that long, then prints; the REPL waits meanwhile. One capture\nruns at a time, on the tab selected when it started. Unlike requests and console it keeps more than\nthe last 200 and lists both together. capture on its own says whether one is running, or shows the last.',
  },
  route: {
    usage: 'route <glob> <status> <json> | off <glob>',
  ...
  modes: {
    ...
    detail: 'The modes are watch, network off, route and capture. Each is turned on and off with its own command:\nwatch on|off, network off|on, route <glob> ... | route off, capture on|off. modes off turns off every\none in every tab; a capture it stops is kept for capture to show.',
  },
